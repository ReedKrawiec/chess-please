...
export class Debug_hud extends HUD {
  setTextElements() {
    ...
    new Text({
      ...
      let mouse = Poll_Mouse(debug_state.camera,false,debug_state.target);
      if(mouse){
...
export let debug_setup = () => {
  ...
  Bind("KeyA",left_func,exec_type.repeat,1,true);
  Bind("KeyD",right_func,exec_type.repeat,1,true);
  Bind("KeyW",up_func,exec_type.repeat,1,true);
  Bind("KeyS",down_func,exec_type.repeat,1,true);
  Bind("scrollup",scroll_up,exec_type.once,1,true);
  Bind("scrolldown",scroll_down,exec_type.once,1,true);
  Bind("KeyS",save_func,exec_type.once,1,true);
  Bind("KeyZ",undo_func,exec_type.once,1,true);
  Bind("KeyQ",()=>{
