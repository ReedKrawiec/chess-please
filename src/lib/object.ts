...
export abstract class obj{
  ...
  opacity:number;
  hud_clickable:boolean = true;
  info:string_map<any> = {}
  text_nodes:Text[] = [];
  cache_entries:string_map<any> = {};
  internal_cache:string_map<any> = {};
  state_properties_to_sync:string[] = [];
  past_state:string_map<string> = {};
  diff_state:string_map<string> = {};
  event_handlers:string_map<void_func[]> = {};
  getState() {
    return this.state;
  }
  ...
  removeEventHandler(event:event_handler){
    this.event_handlers[event.type].splice(event.index,1);
  }
  addEventHandler(event:string,func:void_func):event_handler{
  ...
  mutate(key:string,value:any){
    (<any>this)[key] = value;
  }
  getRoom<T>():T{
    return this.game.getRoom() as unknown as T;
  }
  ...
  bindControl(key: string, x: exec_type, func: control_func, interval = 1) {
    this.binds.push(Bind(key, func, x, interval, false, this));
  }
  ...
  statef(time:number){
    ...
    if(this.state_properties_to_sync.length > 0){
      let network_properties:string_dict<string> = {}
