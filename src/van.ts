...
import { ExecuteRepeatBinds, Unbind, handleGamepad } from "./lib/controls";
...
export const createItemString = (name:string,state_config:obj_state,parameters?:any):obj => {
  if(prefabs[name]){
    let new_obj = <obj>(new prefabs[name](Object.assign({},state_config),parameters));
    return new_obj;
  }
...
export class game<T>{
  ...
  async render(t: number) {
    ...
    for (let a = 0; a < all_cameras.length; a++) {
      ...
      let render_toggles = debug_state ? debug_state.render_toggles : undefined;
      ...
      for (let a of camera_colliders) {
        ...
        if (DEBUG && a.collision && render_toggles["bounding_box"]) {
      ...
      if (camera.state.debug) {
        ...
        while (filled_boxes.length > 0){
          ...
          rect_renderer(this.offscreen_context, rect, box.box.x,box.box.y,box.color,camera,box.opacity);
        }
        ...
        if (DEBUG && debug_state.selected_properties_element) {
          ...
          rect_renderer(this.offscreen_context, { width: 5, height: 5 }, coll.x, coll.y, "skyblue", camera);
